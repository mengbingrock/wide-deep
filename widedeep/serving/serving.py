...
class MatrixSlowServingService(serving_pb2_grpc.MatrixSlowServingServicer):
    ...
    def __init__(self, root_dir, model_file_name, weights_file_name):
        ...
        self.input_node = get_node_from_graph("Tensor:0")
        assert self.input_node is not None
        assert isinstance(self.input_node, Tensor)

        self.input_dim = self.input_node.shape

        self.output_node = get_node_from_graph("Step:6")
    ...
    def _inference(self, inference_req):
        ...
        for mat in inference_req:
            # 将数据输入模型并执行前向传播
            self.input_node.set_value(mat)
            self.output_node.forward()

            # 把输出节点的值作为结果返回
            inference_resp_mat_list.append(self.output_node.outputs)
