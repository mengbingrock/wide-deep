...
sys.path.append('.')
import argparse
from trainer.trainer import DistTrainerParameterServer
from trainer.dist import ps
...
def train(worker_index):
    ...
    # 使用PS训练器，传入集群配置信息
    trainer = DistTrainerParameterServer([x], label, loss, optimizer,
    ...
    saver.save(model_file_name='dist_model.json', weights_file_name='dist_weights.npz')
