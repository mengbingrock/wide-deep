...
from .saver import get_node_from_graph
    ...
    def forward_backward(self):
        ...
        for node in self.graph.nodes:
            if isinstance(node, Tensor) and node.trainable:
                ...
                gradient = node.jacobian.T.reshape(node.shapes())
    ...
    def _update(self):
        ...
        for node in self.graph.nodes:
            if isinstance(node, Tensor) and node.trainable:
                ...
                # 用朴素梯度下降法更新变量节点的值
                node.set_value(node.outputs - self.learning_rate * gradient)